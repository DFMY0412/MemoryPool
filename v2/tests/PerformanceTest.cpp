...
using namespace my_memoryPool_v2;   
...
private:
    struct TestStats
    {
        ...
        double speedupPercent{0.0}; // 提速百分比
...
public:
    ...
    static void testSmallAllocation()
    {
        ...
        // 计算提速百分比：((优化前性能值 - 优化后性能值) / 优化前性能值) * 100%
        if (stats.systemTime > 0) {
            stats.speedupPercent = ((stats.systemTime - stats.memPoolTime) / stats.systemTime) * 100;
            std::cout << "Speedup: " << std::fixed << std::setprecision(2)
                      << stats.speedupPercent << "%" << std::endl;
        }
    }
    ...
    static void testMixedSizes()
    {
...

        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i)
            {
                ...
                void* ptr = MemoryPool::allocate(size);
                sizePtrs[i % sizePtrs.size()].push_back({ptr, size});
